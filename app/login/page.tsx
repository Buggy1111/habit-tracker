  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 429) {
        toast.error(t("tooManyAttempts", { seconds: data.retryAfter || 60 }))
        return
      }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8">
        ...
        <Card className="border-dashed">
          <CardHeader>
            <CardTitle className="text-sm">{t("demoAccount")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-xs text-muted-foreground">
            <p>
              {t("demoEmail")} <span className="font-mono">demo@habittracker.cz</span>
            </p>
            <p>
              {t("demoPassword")} <span className="font-mono">demo123</span>
            </p>
          </CardContent>
        </Card>
      </div>
