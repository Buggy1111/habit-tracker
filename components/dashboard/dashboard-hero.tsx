...
export function DashboardHero({
  ...
  const getMotivationalMessage = () => {
    ...
    if (percentage === 100) {
      return "Skvělá práce! Všechny dnešní návyky splněny! 🎉"
    } else if (percentage >= 75) {
      return "Skoro tam! Ještě pár návyků a máš to! 💪"
    } else if (percentage >= 50) {
      return "Dobrá práce! Pokračuj dál! 🔥"
    } else if (percentage > 0) {
      return "Výborný začátek! Každý krok se počítá! ⭐"
    } else if (totalHabits > 0) {
      return "Nový den, nové příležitosti! Pojďme na to! 🌅"
    } else {
      return "Vytvoř svůj první návyk a začni budovat lepší verzi sebe! ✨"
    }
  }
      ...
      <div className="relative p-4 sm:p-6 lg:p-8">
        ...
        {totalHabits > 0 && (
          ...
          >
            ...
            >
              <span className="font-medium">{completedToday} dokončeno ✓</span>
              <span>{totalHabits - completedToday} zbývá</span>
            </motion.div>
          </motion.div>
        )}
      </div>
