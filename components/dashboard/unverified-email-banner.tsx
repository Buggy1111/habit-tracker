  ...
  async function handleResendEmail() {
    ...
    try {
      ...
      if (response.ok && data.success) {
        toast.success("Verifikační email byl odeslán. Zkontroluj svou schránku!")
      } else {
    ...
    <Alert className="relative border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950">
      ...
      <AlertDescription className="flex items-center justify-between gap-4">
        <div className="flex-1">
          <p className="text-sm font-medium text-amber-900 dark:text-amber-100">Ověř svůj email</p>
          <p className="mt-1 text-sm text-amber-700 dark:text-amber-300">
            Poslali jsme ti ověřovací odkaz na <span className="font-medium">{email}</span>.
            Zkontroluj svou schránku a klikni na odkaz pro ověření účtu.
          </p>
        </div>
