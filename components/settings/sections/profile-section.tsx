    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Email nelze změnit. Pro změnu emailu kontaktujte podporu.
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea id="bio" placeholder="Napište něco o sobě..." rows={4} {...register("bio")} />
        {errors.bio && <p className="text-sm text-destructive">{errors.bio.message}</p>}
        <p className="text-xs text-muted-foreground">Maximum 500 znaků</p>
      </div>

      {/* Timezone */}
      <div className="space-y-2">
        <Label htmlFor="timezone">Časová zóna</Label>
        <Select
          value={selectedTimezone}
          onValueChange={(value) => setValue("timezone", value, { shouldDirty: true })}
        >
          <SelectTrigger id="timezone">
            <SelectValue placeholder="Vyberte časovou zónu" />
          </SelectTrigger>
          <SelectContent>
            {TIMEZONES.map((tz) => (
      ...
      <div className="flex items-center justify-between pt-4 border-t">
        <div>
          {isDirty && (
            <p className="text-sm text-orange-600 dark:text-orange-400">Máte neuložené změny</p>
          )}
        </div>
